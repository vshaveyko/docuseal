...
# Webhook registration, deliberately OUTSIDE the block above.
#
# A deployment that provisions its accounts through
# Api::Internal::ProvisionAccountsController sets no DOCUSEAL_ADMIN_EMAIL /
# DOCUSEAL_ADMIN_PASSWORD — it has no seed admin to create. Webhook seeding used
# to live inside that guard, so those deployments registered no callback at all:
# ProcessSubmitterCompletionJob found nothing in `WebhookUrls.for_account_id`,
# never enqueued SendFormCompletedWebhookRequestJob, and the embedding app was
# never told the form had been signed. It is its own concern with its own env
# vars, so it gets its own guard.
Rails.application.config.after_initialize do
  next if EnvWebhookSeed.configured_url.blank?

  begin
    next unless ActiveRecord::Base.connection.data_source_exists?('webhook_urls')
  rescue ActiveRecord::NoDatabaseError, ActiveRecord::ConnectionNotEstablished,
         ActiveRecord::StatementInvalid, PG::Error
    next
  end
  ...
  begin
    EnvWebhookSeed.call_all
  rescue StandardError => e
    Rails.logger.warn("[env_seed] webhook seeding skipped: #{e.class}: #{e.message}")
  end
end
