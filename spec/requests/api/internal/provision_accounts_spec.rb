...
describe 'Internal Provision Account API' do
  ...
  describe 'POST /api/internal/provision_account' do
    context 'with the secret configured' do
      ...
      # Without this the tenant has no callback until the next boot reseeds it,
      # so every form signed in between completes silently and the embedding app
      # keeps showing the document unsigned.
      it 'registers the env webhook on the new account' do
        allow(ENV).to receive(:[]).with('DOCUSEAL_WEBHOOK_URL')
                                  .and_return('https://app.example.com/webhooks/docuseal?token=s3cret')

        post '/api/internal/provision_account', headers: { 'X-Provision-Token': sign_token(valid_payload) }

        webhook = User.find_by(email: 'owner@example.com').account.webhook_urls.sole

        expect(webhook.url).to eq('https://app.example.com/webhooks/docuseal?token=s3cret')
        expect(webhook.events).to include('form.completed')
      end

      it 'provisions normally when no webhook is configured' do
        allow(ENV).to receive(:[]).with('DOCUSEAL_WEBHOOK_URL').and_return(nil)
